...
I built a time-domain power-budget model of the baseline vehicle before looking at any harvester,
because "would this help?" has no answer until you know how large the deficit is and *when* it
occurs. The model runs an hourly energy balance over a year: solar geometry (declination,
elevation — so day length and polar night emerge naturally), Kasten–Young air mass, the cosine loss
of a low sun onto a deck panel, persistent stochastic cloud, sail shading, and a battery with
temperature-derated capacity, a sub-zero charge inhibit and ageing. All assumptions are labelled
`A1`–`A25` inline in `cstar_power_model.py`, each with a note on how to firm it up.

Throughout, **safe mission duration** means time from launch until the battery first falls below
20 % of nominal — not the point of failure, but the point at which no reserve remains for a storm
or a bad weather run. All runs assume a **1 June launch**: it exercises a full seasonal cycle, it is
the most favourable date so every failure is a genuine capability limit, and a vehicle reaching
spring with charge in hand returns to surplus conditions and can then run indefinitely.
...
July. Three effects compound, all from low sun: day length collapses; the flat deck panel suffers a
cosine loss (at a 1.9° sun, 97 % of available light is lost to geometry alone); and the beam crosses
ten atmospheres instead of one. This is a **seasonal storage problem**, so a useful solution must
deliver *in the dark months specifically*, not on annual average.
...
The sweep stops at 74 °N because the Arctic basin is ice-covered beyond roughly 75 °N; a 1 m sailing
hull cannot operate in ice, the model has no ice physics, and December generation has already
saturated at zero. Ice would also damp the wave field, which matters for one option below.
...
**Why margin matters beyond survival.** C-Star also operates in driven mode under propeller, and an
operator will only use that capability if they can trust the vehicle to recharge. A solution that
merely scrapes through winter is worth much less than one that restores usable margin.
...
complete vehicle. This is not a modification but a different vehicle. It is the right answer for the
...
(50–150 m), so it is a *wave follower*: it rides the surface, and there is no hull-to-water relative
...
stroke and 50 % PTO efficiency, mean output is **0.023 W**, about 2 % of the load, and the latitude
curve is indistinguishable from solar alone. Power is directly proportional to stroke, and reaching
0.5 W would need roughly a 5 kg mass moving 0.5 m inside a 1 m vehicle. **This is a scale problem,
not a maturity problem** — no PTO refinement recovers a factor of twenty. Recommend closing it out.
...
antiphase with the sun — the physical basis of the whole concept. Wind is modelled as a persistent
AR(1) daily series, shear-corrected from 10 m to a ~1 m hub (costing ~22 %), with the cubic
evaluated hourly because E[v³] is roughly twice E[v]³.

Mean output is **2.6 W at 60 °N and 3.1 W at 70 °N** — five to ten times requirement, and 2–3x the
entire vehicle load.
...
2. **Instrument a hull already going to sea (weeks, low cost).** IMU, GPS speed and panel current.
   Retires three assumptions at once: the acceleration spectrum settles wave definitively, the speed
   distribution settles regeneration, and panel current gives real shading loss.
...
With **six months** the first two months are unchanged — those tests are on the critical path either
way and their value does not scale with schedule. The extra four buy: a sealed, bearing-life-tested
installation; the icing and knockdown survival campaign; a winter deployment above 60 °N validating
the model against reality rather than reanalysis; and re-qualification of vehicle stability with
mass aloft. The deliverable is a prototype Oshen could sell a mission on.
